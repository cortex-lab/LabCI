...
describe('coverage endpoint', () => {
    ...
    before(function (done) {
        ...
        fs.mkdir(reportsDir, {recursive: true}, async (err) => {
            if (err) throw err;
            await fs.writeFile(path.join(reportsDir, 'foobar.log'), '', (err) => {
                if (err) throw err;
            });
            await fs.writeFile(path.join(reportsDir, 'index.html'), '', (err) => {
                if (err) throw err;
            });
            done();
        });
...
describe('queue finish callback', () => {
    ...
    it('test error handling', (done) => {
        queue.process(async (job) => {
            job.done(new Error('foobar'));
        });  // Raise error
        queue.on('error', _ => {
        });  // Error to be handles in finish callback
        const data = {
            sha: SHA,
            skipPost: false,
            context: 'coverage',
